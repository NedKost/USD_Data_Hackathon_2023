...
def preprocessed_data(is_training):
    if is_training:
        data = Path("data/fars_train.csv")
    else:
        data = Path("data/fars_test.csv")
    
    
    # Load data from Dataframe
    df = pd.read_csv(data, quoting=1, delimiter=',')
